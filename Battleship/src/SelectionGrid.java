...
public class SelectionGrid extends Rectangle {
    ...
    private void initilizeTreasures(int numOfTreasures) {
        while (treasures.size() < numOfTreasures) {
            int x = rand2.nextInt(GRID_WIDTH);
            int y = rand2.nextInt(GRID_HEIGHT);
    ...
    private void placeTreasure(int x, int y) {
        // Assuming treasures is a list of Position objects representing treasure locations
        treasures.add(new Position(x, y));
        markers[x][y].setAsTreasure(new Position(x, y));
    }
